...
export class DateTimeMutation {
    ...
    public static set(time: number, ...intervals: [DateTimeKind, number][]) {
        return this.apply(setters, time, intervals);
    }

    public static mutate(time: number, ...mutations: [DateTimeKind, number][]) {
        return this.apply(mutators, time, mutations);
    }

    protected static apply(map: Map<DateTimeKind, TimeMutateFn>, time: number, values: [DateTimeKind, number][]) {
